...
@RestController
public class MailController { //для обработки входящих http-запросов (POST-запросы для отправки данных на сервер)
    ...
    public ResponseEntity<?> sendActivationMail(@RequestBody MailParams mailParams) { //получение входящих POST-запросов
        //ResponseEntity<?> - Spring-класс для удобной сборки http-ответа, @RequestBody - доступ к телу http-запроса
